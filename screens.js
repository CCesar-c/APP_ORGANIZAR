...
function Inicio() {
  ...
  async function deleteItem(i, nome_tarea) {
    const notificaciones_agendadas = await Notifications.getAllScheduledNotificationsAsync()
    const tarea_agendada = notificaciones_agendadas.find(n => n.content.body.includes(nome_tarea));

    for (let i = 0; i < notificaciones_agendadas.length; i++) {
      await Notifications.cancelScheduledNotificationAsync(tarea_agendada[i].identifier);
    }

    console.log("tarea borrada: ", tarea_agendada[0].content.body);
    // Al crearla:
    ...
    try {
      ...
      if (Platform.OS === 'android') {
        for (let i = 0; i < horas.length; i++) {
          ...
          const nueva_tarea = {
            id: id_temporal, // Guardamos el ID de la notificación para poder cancelarla después
            nome: nome,
